...
public class RobotContainer {
        ...
        private final CommandXboxController driver =
                        new CommandXboxController(OperatorConstants.kDriverJoystickPort);
        private final CommandXboxController operator =
                        new CommandXboxController(OperatorConstants.kOperatorJoystickPort);

        // JOYSTICKS
        // END-------------------------------------------------------------------------------------------------------------------

        // SUBSYSTEMS----------------------------------------------------------------------------------------------------------------------
        private final XboxController driverRaw =
                        new XboxController(OperatorConstants.kDriverJoystickPort);
        private final XboxController operatorRaw =
                        new XboxController(OperatorConstants.kOperatorJoystickPort);
        ...
        private final LEDSubsystem ledSubsystem = new LEDSubsystem(beamBreakSubsystem,
                        shooterSubsystem, intakeSubsystem, shooterWrist, operatorRaw);
        private final RunIntakeWithJoystick runIntakeWithJoystick =
                        new RunIntakeWithJoystick(intakeSubsystem, operator);
        private final RunClimberWithJoystick runClimberWithJoystick =
                        new RunClimberWithJoystick(climber, operator);
        ...
        private final RunShooterWristWithJoystick runShooterWristWithJoystick =
                        new RunShooterWristWithJoystick(shooterWrist, operator);
        private final RunShooterWithPID runShooterWithPID =
                        new RunShooterWithPID(shooterSubsystem, 5500, 3000);
        ...
        private final SwerveRequest.FieldCentricFacingAngle fieldAngle =
        ...
        private void registerCommands() {
                ...
                NamedCommands.registerCommand("ShootNoteMid", new ShootNote(shooterWrist,
                                shooterSubsystem, drivetrain, intakeSubsystem, 66.0 / 360));
                NamedCommands.registerCommand("ShootNoteMidFar", new ShootNote(shooterWrist,
                                shooterSubsystem, drivetrain, intakeSubsystem, 39.0 / 360));
                NamedCommands.registerCommand("ShootNoteFar", new ShootNoteFar(shooterWrist,
                                shooterSubsystem, drivetrain, intakeSubsystem, 37.5 / 360));
                NamedCommands.registerCommand("ShootNoteDown", new ShootNote(shooterWrist,
                                shooterSubsystem, drivetrain, intakeSubsystem, 33.0 / 360)); // change
                                                                                             // this
                NamedCommands.registerCommand("ShootNoteLeft", new ShootNote(shooterWrist,
                                shooterSubsystem, drivetrain, intakeSubsystem, 62.0 / 360));
                NamedCommands.registerCommand("ShootNoteRight", new ShootNote(shooterWrist,
                                shooterSubsystem, drivetrain, intakeSubsystem, 62.0 / 360));
                NamedCommands.registerCommand("IntakeNote",
                                new IntakeNote(intakeSubsystem, driverRaw, operatorRaw));
                NamedCommands.registerCommand("RevShooter", new RunCommand(() -> {
                        shooterSubsystem.setRPM(3500, 3500);
                }, shooterSubsystem));
        }

        public Command funnelRotation() {
                boolean isBlueAlliance = DriverStation.getAlliance().orElse(Alliance.Blue)
                                .equals(Alliance.Blue);
                double angle = (isBlueAlliance ? 20 : -20) * Math.PI / 180.0;
                return drivetrain.applyRequest(() -> fieldAngle
                                .withVelocityX(-driver.getLeftY() * RobotConstants.kMaxSpeed)
        ...
        private void configureBindings() {
                ...
                drivetrain.setDefaultCommand( // Drivetrain will execute this command periodically
                                drivetrain.applyRequest(() -> drive
                                                .withVelocityX(-driver.getLeftY()
                                                                * RobotConstants.kMaxSpeed) // Drive
                                                // forward with
                                                // negative Y (forward)
                                                ...
                                                .withRotationalRate(-driver.getRightX()
                                                                * RobotConstants.kMaxAngularRate) // Drive
                                                                                                  // counterclockwise
                                                                                                  // with
                                // negative X (left)
                                ));
                ...
                driver.leftBumper().onTrue(drivetrain.runOnce(() -> drivetrain.seedFieldRelative(
                                new Pose2d(new Translation2d(1.5, 15.5), new Rotation2d()))));
                ...
                // );
                driver.x().whileTrue(drivetrain.applyRequest(() -> fieldAngle
                                .withVelocityX(-driver.getLeftY() * RobotConstants.kMaxSpeed)
                                .withVelocityY(-driver.getLeftX() * RobotConstants.kMaxSpeed)
                ...
                operator.leftBumper()
                                .onTrue(new IntakeNote(intakeSubsystem, driverRaw, operatorRaw));
                ...
                driver.a().and(driver.povUp())
                                .whileTrue(drivetrain.runDriveQuasistaticTest(Direction.kForward));
                driver.a().and(driver.povDown())
                                .whileTrue(drivetrain.runDriveQuasistaticTest(Direction.kReverse));

                driver.x().and(driver.povUp())
                                .whileTrue(drivetrain.runDriveDynamicTest(Direction.kForward));
                driver.x().and(driver.povDown())
                                .whileTrue(drivetrain.runDriveDynamicTest(Direction.kReverse));

                driver.y().and(driver.povUp())
                                .whileTrue(drivetrain.runSteerQuasistaticiTest(Direction.kForward));
                driver.y().and(driver.povDown())
                                .whileTrue(drivetrain.runSteerQuasistaticiTest(Direction.kReverse));

                driver.b().and(driver.povUp())
                                .whileTrue(drivetrain.runSteerDynamicTest(Direction.kForward));
                driver.b().and(driver.povDown())
                                .whileTrue(drivetrain.runSteerDynamicTest(Direction.kReverse));
